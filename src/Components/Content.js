...
const Content = () => {
  ...
  const getSlug = location.pathname
    .slice(1, location.pathname.length - 1)
    .split("-");
  const getSlugUpperCase = location.pathname
    .replace(/-/g, " ")
    .slice(1, location.pathname.length - 1)
    .toUpperCase();
            ...
            <div className="w-full h-full justify-center flex flex-wrap">
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: sanitize(content.title.rendered),
                }}
              ></div>
              <div
                className="w-1/2 nest text-justify mt-5"
                dangerouslySetInnerHTML={{
                  __html: sanitize(content.content.rendered),
                }}
              ></div>
