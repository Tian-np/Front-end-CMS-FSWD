...
const Comments = ({post_id}) => {
    ...
    return (
        <>
                        ...
                        <div key={index} className='mb-3.5 w-full flex justify-center'>
                            <div className='w-1/4 border border-white rounded-md flex flex-wrap justify-center '>
                                <div className='w-full h-1/2 flex mb-2 p-2'>
                                    ...
                                    <div dangerouslySetInnerHTML={{__html:sanitize(comment.author_name)}} className='pl-3.5'></div> 
                                </div>
                                <div className='w-full flex justify-start pb-4'>
                                    <div dangerouslySetInnerHTML={{__html:sanitize(comment.content.rendered)}} className='pl-5'></div> 
                                </div>
            ...
            <form onSubmit={handleOnSubmit} className='w-full h-1/4 bg-gray-300 p-5 flex justify-center'>
                <div className='h-full w-3/4'>
                    <input className='w-full h-1/3 border border-black p-4 rounded-md bg-gray-500 focus:duration-150 focus:transition hover:bg-gray-700 focus:bg-white focus:outline-none mb-3' placeholder='Input name ' onChange={(e) => setName(e.target.value)} value={name} />
                    <div className='w-full flex h-1/2'>
                        <input className='w-full h-2/3 border border-black p-4 rounded-md bg-gray-500 hover:bg-gray-700 focus:duration-150 focus:transition focus:bg-white focus:outline-none' placeholder='Create comment' onChange={(e) => setComment(e.target.value)} value={comment} />
                        <button type='submit' className='text-white bg-gray-600 h-2/3 ml-4 rounded-md duration-150 transition p-2 hover:bg-gray-900' >Submit</button>
                    </div>
                </div>
